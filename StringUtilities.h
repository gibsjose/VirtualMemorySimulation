...
public:
    ...
    static T StringToNumber(const std::string& numberAsString) {
        ...
        stream >> valor;
        if (stream.fail()) {
            throw ParseException("Could not convert string " + numberAsString + " to a number");
        }
        return valor;
    }
    ...
    static unsigned int GetIndexOfStringInVector(std::vector<std::string> v, std::string s) {
        ...
        if(index >= v.size()) {
            throw GeneralException("StringUtilities::GetIndexOfStringInVector: String not found in vector: Index out of bounds");
        } else {
            return index;
        }
    }
